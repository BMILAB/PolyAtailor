...
scan_findumi <- function(x,mcans,findUmi,lumi,adapterSeq,anchorSeq,mapping,tailAnchorLen,findTailType){
  #Initialisation
  read_nums <- c()
  strand <- c()
  umis <- c()
...
#'   
#' @usage
#' 1) Identify multiple tails (including both A-tail and T-tail) in a read
#'    (recommended for PacBio data)
#' > tailScan(fastqfile,mcans=5,findUmi=F,resultpath="",samplename=
#'    "",tailAnchorLen=8,minTailLen=8,realTailLen=20,maxNtail=3,mapping=F)
#' 2) Identify multiple tails in a read and set specific adapters (recommended
#'    for PacBio data such as FLAM-seq)
#' > tailScan(fastqfile,mcans=5,findUmi=F,resultpath="",samplename=
#'    "",tailAnchorLen=8,minTailLen=8,realTailLen=20,maxNtail=3,mapping=F,
#'    adapterSeq="ACTG")
#' 3) Identify a maximum of one A-tail for each read (recommended for NGS data
#'    such as PAT-seq)
#' > tailScan(fastqfile,mcans=5,findUmi=F,resultpath="",samplename=
#'    "",tailAnchorLen=8,minTailLen=8,realTailLen=20,maxNtail=1,mapping=F,
#'    findTailType='A')
#' 
#' @examples
  ...
  if(longRead){
    if(directBam){
      ...
      allcigars2 <- allcigars2 %>%
        mutate(anchor1=str_detect(seq,pattern1),
      ...
      allcigars21 <- allcigars21 %>%
        ...
        select(read_num,chr,strand,coord,PAL,tail,anchor1,anchor2)
      ...
      allcigars3 <- allcigars3 %>%
        select(read_num,chr,strand,coord,tail,anchor1,anchor2) %>%
        rename(seq=tail)
      ...
      if(findUmi){
        allcigars3 <- allcigars3 %>%
          ...
          select(read_num,chr,strand,coord,PAL,tail,nA,rt,anchor1,anchor2) %>%
