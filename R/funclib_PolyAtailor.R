...
#' @examples
#' fastqfile <- "./inst/extdata/GV_fastq/PAIso_GV1.fastq"
#' faBuilderRE <- faBuilder(fastqfile,mcans=5,findUmi = F,resultpath = "./inst/extdata/output/",samplename =
  ...
  if(longRead){
    if(directBam){
      ...
      allcigars2 <- allcigars2 %>%
        dplyr::mutate(anchor1=str_detect(seq,pattern1),
      ...
      allcigars21 <- allcigars21 %>%
        dplyr::mutate(str=str_extract(cigar,"[0-9]+S"),
